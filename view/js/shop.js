...
function loadDetails(id) {
    ajaxPromise('module/shop/controller/controller_shop.php?op=read_car&id=' + id, 'GET', 'json')
        .then(function (data) {
            ...
            $('#details-ov-overv').html(data['data'].overview_car);
            $('<div></div>').attr('id', 'details-ov-shopd').addClass('cont-just mt-3').appendTo('#details-row-cont-ov')
            $('#details-ov-shopd').html('<h2>' + data['data'].price_car + ' €</h5>');
            ...
            $('#details-table-tbody').html(function () {
                ...
                for (row in data['data']) {
                    ...
                    if (row != "id_car" && row != "overview_car" && row != "created_at" && row != "updated_at") {
                        content += '<tr><td width="390">' + row + '</td><td>' + data['data'][row] + '</td></tr>';
                    }
                }
                return content;
            })
            ...
            markers.push({
                ...
                text: data['data'].name_mark + ' ' + data['data'].name_model,
                price: data['data'].price_car,
                coord: [data['data'].lng, data['data'].lat]
            })
            setMarks(markers)
            loadReleatedDetails(data['data'].id_mark, id)
        }).catch(function (e) {
            console.log("error" + e);
        })
}
